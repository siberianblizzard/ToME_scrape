...
find a way to calculate amount of pages
add a function for calculating a total amount of characters for the diff+mode
'''
...
def count_classes(mode='roguelike', diff='normal'):
    winners_list = get_winners(mode, diff)
    ...
    for winner in winners_list:
        if winner is not None:
            winners_class = re.search(r'\s\d+\s\w+\s(.*)', winner)
            if winners_class is not None:  # for broken non-unicode char sheets
                for class_name in class_list:
                    if class_name in winners_class[1]:
                        class_stats[class_name] += 1
                        break
    df = pd.DataFrame.from_dict(class_stats, orient='index')
    df.to_excel('winners_{0}_{1}.xlsx'.format(mode, diff), header=True)
    return df


print(count_classes('roguelike', 'normal'))
